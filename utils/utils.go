...
func NumsToBHex(nums []uint64) string {
	num_str := ""
	for idx, num := range nums {

		num_str += strconv.FormatUint(num, 35)
		if idx != len(nums) - 1 {
			num_str += "Z"
		}
	}
...
func BHex2Nums(str string) []uint64 {
	str = strings.ToLower(str)
	strs := strings.Split(str, "z")
	var nums []uint64
	for _, s := range strs {
		num, err := strconv.ParseUint(s, 35, 64)
		if err != nil {
			return nil
		}
