...
func New(st interface{}) (*CsvFile, error) {
...
func (rf *CsvFile) Read(name string) error {
...
func (rf *CsvFile) Record(i int) interface{} {
	return rf.records[i]
}

func (rf *CsvFile) NumRecord() int {
	return len(rf.records)
}

func (rf *CsvFile) Indexes(i int) Index {
	if i >= len(rf.indexes) {
		return nil
	}
	return rf.indexes[i]
...
func (rf *CsvFile) Index(i interface{}) interface{} {
	index := rf.Indexes(0)
	if index == nil {
		return nil
	}
...
func ReadCsv(path string, st interface{}) (*CsvFile, error) {
	rf, err := New(st)
	if err != nil {
		return nil, err
	}
